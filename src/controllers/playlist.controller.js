...
export const createPlaylist = asyncHandler(async (req, res) => {
  ...
  if (!newplaylist)
    throw new ApiError(502, "There was a error while creating a playlist !! ");
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, newplaylist, "Playlist Fetched Successfully !!")
    );
});
...
export const updatePlaylist = asyncHandler(async (req, res) => {
  ...
  const { id } = req.params;
  if (!(name && description))
    throw new ApiError(200, "Name and Discription is required !!");
  ...
  if (!updatePlaylist)
    throw new ApiError(502, "Credentials Were Not Updated !! ");

  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedPlaylist, "Playlist Credentials Updated ")
    );
});
...
export const getAllPlaylistById = asyncHandler(async (req, res) => {
  ...
  const FetchedAllplaylist = await Playlist.aggregate([
    {
      $match: {
  ...
  console.log("FetchedPlaylist:  "+FetchedAllplaylist.length);
  
  if (!FetchedAllplaylist) throw new ApiError(402, "Playlist Not Found !");
  return res
    .status(200)
    .json(
      new ApiResponse(200, FetchedAllplaylist, "Playlist Fetched SuccessFully")
    );
});

export const DeleteVedioFromPlaylist = asyncHandler(async (req, res) => {
  const { vedioId, PlaylistId } = req.params;
  if (!(vedioId && PlaylistId))
    throw new ApiError(400, "Please Provide Vedio and Playlist !!");

  const PlaylistFromdb = await Playlist.findById(PlaylistId);
  if (!PlaylistFromdb) throw new ApiError(402, "Playlist Not found !!");

  PlaylistFromdb.vedios = PlaylistFromdb.vedios.filter(
    (id) => id.toString() !== vedioId.toString()
  );
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, {}, "Vedio Deleted From Playlist Successfully ")
    );
});
...
export const AddVedioToPlaylist = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, {}, "Vedio Added To Playlist Successfully "));
});
